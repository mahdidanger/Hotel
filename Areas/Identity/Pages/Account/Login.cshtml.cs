        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (user != null)
                    {
                        // دریافت مقدار IsModir
                        var isModir = user.IsModir;

                        // ساختن claim برای IsModir
                        var claims = new List<Claim>
                {
                    new Claim(ClaimTypes.Name, user.UserName),
                    new Claim("IsModir", isModir.ToString()) // اضافه کردن IsModir به عنوان claim
                };

                        var claimsIdentity = new ClaimsIdentity(claims, IdentityConstants.ApplicationScheme);

                        // اضافه کردن claim به session
                        await HttpContext.SignInAsync(IdentityConstants.ApplicationScheme, new ClaimsPrincipal(claimsIdentity));

                        _logger.LogInformation("User logged in with IsModir status.");
                    }
